...
const mapDispatchToProps = (dispatch) => ({
  onLoad: (payload) => dispatch({ type: SYNTHESISDOC_PAGE_LOADED, payload }),
});
...
class SynthesisDoc extends Component {
  componentDidMount() {
    if (this.props.match.params) {
      return this.props.onLoad(
        patientRequest.getPatientSynthesisById(this.props.match.params.id)
      );
    }
  ...
  render() {
    const { synthesisDoc } = this.props;
    if (synthesisDoc && synthesisDoc.data) {
      return (
        <div>
          <img
            src={require('./synthesis_header.png')}
            width="100%"
            alt="chuo-service-infectieux"
          />
